#!/usr/bin/env node
// Regenerates src/esi/miningData.json from CCP's official Static Data Export (SDE).
//
// The list of mineable ore, ice and gas types (and their packaged volumes) is
...
//
//   node scripts/generate-mining-data.mjs
//
...
const OUTPUT_PATH = fileURLToPath(new URL("../src/esi/miningData.json", import.meta.url));
...
// "Asteroid" category, with ice being the single group named "Ice" within it; every mineable
...
export async function generate(zipBuffer) {
  ...
  const gasGroup = findByName(groups, GAS_GROUP_NAMES, "a mineable gas group");
  ...
  console.log(`Wrote ${data.length} mining types to ${OUTPUT_PATH}`);
}
